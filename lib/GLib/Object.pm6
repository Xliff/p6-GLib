...
role GLib::Roles::Signals::GObject does GLib::Roles::Signals::Generic {
  ...
  method connect-notify (
    ...
    self.signal{$signal}.tap(&handler) with &handler;
    self.signal{$signal};
  }
...
class GLib::Object {
  ...
  submethod BUILD (:$object) {
    ...
    $!signals = GLib::Object::SignalContainer.new;
  }
  ...
  multi method new (Int() $type, :$ptr is required) {
    my GType $t = $type;
  ...
  method !set-data-abstract(@pairs, ::T, ::NT, $value, &f) {
  ...
  {
    my $hid = GLib::Signal.connect_closure(
      $!o,
      $detailed_signal,
      $closure,
      $after
  ...
  {
    GLib::Signal.connect_closure_by_id(
      $!o,
      $signal_id,
      $detail,
      $closure,
    ...
    # cw: - XXX -
    #     This is also a nightmare. Must search $!signals for $hid,
    #     clear any preserved callable, and then preserve the closure!
  }
  ...
  method connect_data (
    ...
    is also<connect-data>
  {
    my $hid = GLib::Signal.connect_data(
      $!o,
      $detailed_signal,
      &c_handler,
      $data,
    ...
    # cw: NOT SUFFICIENT!
    %!signals{$detailed_signal}[2]        = $hid;
    %!signals{$detailed_signal}<callable> = &c_handler;
  }
  ...
  {
    my $hid = GLib::Signal.connect_after(
      $!o,
      $detailed_signal,
      &c_handler,
      $data
  ...
  {
    my $hid = GLib::Signal.connect_swapped(
      $!o,
      $detailed_signal,
      &c_handler,
      $data
  ...
  method disconnect_by_func (
    ...
    GLib::Signal.disconnect_by_func($!o, $func, $data);
  }

  method get_invocation_hint
    is also<
      get-invocation-hint
      invocation_hint
      invocation-hint
    >
  {
    GLib::Signal.get_invocation_hint($!o);
  }

  method handler_block (GObject() $instance, Int() $handler_id)
    is also<handler-block>
  {
    GLib::Signal.handler_block($!o, $handler_id);
  }

  method handler_disconnect (Int() $handler_id)
    is also<handler-disconnect>
  {
    GLib::Signal.handler_disconnect($!o, $handler_id);
  }
  ...
  method handler_is_connected (Int() $handler_id)
    is also<handler-is-connected>
  {
    GLib::Signal.handler_is_connected($!o, $handler_id);
  }

  method handler_unblock (Int() $handler_id)
    is also<handler-unblock>
  {
    GLib::Signal.handler_unblock($!o, $handler_id);
  }
  ...
  {
    GLib::Signal.handlers_block_matched(
      $!o,
      $mask,
      $signal_id,
      $detail,
  ...
  method handlers_destroy is also<handlers-destroy> {
    GLib::Signal.handlers_destroy($!o);
  }
  ...
  {
    GLib::Signal.handlers_disconnect_matched(
      $!o,
      $mask,
      $signal_id,
      $detail,
  ...
  {
    GLib::Signal.handlers_unblock_matched(
      $!o,
      $mask,
      $signal_id,
      $detail,
  ...
  {
    GLib::Signal.has_handler_pending($!o, $signal_id, $detail, $may_be_blocked);
  }
  ...
  {
    GLib::Signal.stop_emission($!o, $signal_id, $detail);
  }

  method stop_emission_by_name (Str() $detailed_signal)
    is also<stop-emission-by-name>
  {
    GLib::Signal.stop_emission_by_name($!o, $detailed_signal);
  }
